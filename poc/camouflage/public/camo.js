...
(function () {
  ...
  function syncControls() {
    /* Folded, the head is all you see of the drawer, so it says what the two
       selects underneath are set to. The dials themselves are DialKit's — it
       paints its own values and needs nothing from here. */
    if (summaryEl) {
  ...
  /* The panel hands down a whole value set at a time. Everything is clamped
     here rather than trusted: these also arrive from the hash, which anyone
     can type. */
  function setValues(v) {
    if (!v) return;
    if (v.palette && COLOURWAYS[v.palette]) state.palette = v.palette;
    if (v.pattern) state.pattern = v.pattern;
    if (v.blur != null) state.blur = clamp(parseFloat(v.blur) || 0, 0, 2);
    if (v.layers != null) state.layers = clamp(parseInt(v.layers, 10) || 4, 2, 6);
    syncControls();
    generate();
  }
  ...
  /* What the DialKit panel talks to. getValues() is read once on mount so the
     dials open on the state the hash asked for; setValues() is the panel
     talking back. */
  window.camo = {
    getValues: function () {
      return {
